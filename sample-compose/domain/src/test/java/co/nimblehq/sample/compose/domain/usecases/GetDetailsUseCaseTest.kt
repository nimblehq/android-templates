...
class GetDetailsUseCaseTest {
    ...
    fun `When request successful, it returns success`() = runTest {
        ...
        val id = 1
        every { mockRepository.getDetails(id) } returns flowOf(expected)

        getDetailsUseCase(id).collect {
    ...
    fun `When request failed, it returns error`() = runTest {
        ...
        val id = 1
        every { mockRepository.getDetails(id) } returns flow { throw expected }

        getDetailsUseCase(id).catch {
            it shouldBe expected
        }.collect()
    }
}
